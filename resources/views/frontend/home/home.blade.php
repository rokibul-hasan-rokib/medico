...
@section('content')
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Set maximum date to 30 days from today in YYYY-MM-DD format
        ...
        // Set min and max attributes (in case a browser respects it)
        dateInput.min = minDate;
        dateInput.max = maxDateString;

        // Use an external library or custom logic to enforce DD-MM-YYYY format if needed

        // Example: restrict selection to allowed range in DD-MM-YYYY
        dateInput.addEventListener('change', function() {
            const selectedDate = new Date(dateInput.value.split('-').reverse().join('-'));

            if (selectedDate < new Date(minDate) || selectedDate > new Date(maxDateString)) {
                alert('Please select a date within the allowed range.');
                dateInput.value = ''; // Clear the input if out of range
            }
        });
    });
</script>
