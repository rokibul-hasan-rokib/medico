...
<ul>
    ...
    <li><strong>Date:</strong> {{ $appointment->date->format('F j, Y') }}</li>
    <li><strong>Department:</strong> {{ $appointment->department }}</li>
    <li><strong>Doctor:</strong> {{ $appointment->doctor }}</li>
</ul>

<p>Please note that this appointment was canceled on {{ $appointment->canceled_at->format('F j, Y, g:i a') }}.</p>
